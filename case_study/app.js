...
const state = { choice: null, revealed: false, investigation: null, caseId: 'alex' };
...
function renderInvestigation() {
  ...
  if (!action) return;
  const heading = action === 'resolve' ? 'Reopen the original conversation.' : 'Ask one question before committing.';
  const question = action === 'probe' ? `<p class="probe-question">${activeCase().probeQuestion || '“Does this memory still apply?”'}</p>` : '';
  $('investigationResult').innerHTML = `<p class="eyebrow">SCRIPTED EVIDENCE REVEAL · ${action.toUpperCase()}</p><h3>${heading}</h3>${question}<div class="resolved-worlds">${currentWorlds().map(world => `<article><span class="small-label">IN WORLD ${world.id}</span><blockquote>${action === 'resolve' ? world.source : world.probe}</blockquote><p>${world.qualified}</p></article>`).join('')}</div><div class="return-path"><span>Request evidence</span><b>→</b><span>Qualify scope & validity</span><b>→</b><span>Reassess authorization</span></div><p class="cost-note">This scripted reveal supplies decisive evidence in each world. A failed or inconclusive check would leave qualification unresolved. Temporary access alone never authorizes persistent keep.</p>`;
}

function renderInspector() {
  $('qualificationState').textContent = state.investigation ? 'New evidence, per world' : 'Unresolved';
  $('authorizationState').textContent = state.investigation ? 'Reassess per world' : (state.choice ? `${state.choice} (visitor choice)` : 'Not selected');
  $('accessState').textContent = state.investigation === 'probe' ? 'Temporary scoped check' : state.investigation === 'resolve' ? 'Source inspection' : 'No probe';
  $('auditTrace').textContent = state.investigation ? `Ambiguous note → defer commitment → ${state.investigation} → new evidence → qualification → conditional authorization.` : `Source note → relevance proposal → ${state.choice || 'decision pending'}${state.revealed ? ' → opposite consequences in two latent worlds' : ''}.`;
}
