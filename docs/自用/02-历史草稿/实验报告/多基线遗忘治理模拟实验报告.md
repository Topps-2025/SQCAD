...
- 组噪声：0.0、0.1、0.2、0.4；
- risk-gated 置信门槛：0.75；item-level 负效应 veto：稳定下界 $\le -0.25$；四档实验使用同一预先固定门槛；
...
以 `causal_item_stable` 为逐 seed 配对强基线，联合方法的差值（均值 $\pm$ 95% CI 半宽）如下：

| 组噪声 | $\Delta$ normalized utility | $\Delta$ rare-critical recall | $\Delta$ stale retention | $\Delta$ positive precision |
| ---: | ---: | ---: | ---: | ---: |
...
原始联合方法的逐 seed 结果说明，高关键召回并不自动形成全指标帕累托优势。为回应这一 bad case，新增 `decomp_abstract_risk_gated`：只有表示置信度达到0.75、且抽象组效应未与可估计 item-level 效应发生符号冲突时，才使用组级分数；item-level 稳定效应的 $LCB \le -0.25$ 时触发负效应 veto，否则回退到 `causal_item_stable`。门槛在四档联合重跑前固定，不按测试噪声单独调节。
