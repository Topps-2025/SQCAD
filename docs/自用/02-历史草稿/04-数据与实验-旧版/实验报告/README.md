...
>
> - **核心实验报告（外部，逻辑顺序）**：`docs/自用/03-实验证据链/` —— ① research gap 佐证实验（命题 A/B/C 反例 + 公平性审查）→ ② 形式化定理与陈述（`docs/自用/01-research-gap/研究逻辑与理论证明/11-形式化定理陈述与证明.md`）→ ③ 充分识别实验 → ④ 必要性与门控下界 → ⑤ 现实、成本与统计支撑。研究逻辑路线见 `docs/自用/01-research-gap/研究路线与方案/00-ResearchGap到充分性与必要性的完整实验逻辑线-20260813.md`。
> - **非核心与旧版本报告（草稿归档）**：`docs/自用/02-历史草稿/实验报告/`（LongMemEval 系列、作用域、解构抽象、最小框架、Gate-A、早期校准与基线报告、Gap v1 旧版等）。
...
`unified_agent_memory_runner.py` 进一步在同一候选流、任务序列、工作区预算和 evaluator 下运行 recency、frequency、fade-like、outcome-feedback-like、item-level causal 与 risk-gated decomposition–abstraction 策略，并将 exposure、adoption、action、outcome 和状态迁移写入上述存储。30-seed 工程 smoke 结果见 `docs/自用/02-历史草稿/实验报告/统一AgentMemory工作流Runner初步实验报告.md`；它不是公开 benchmark 结果。完整框架与实验合同见 `docs/自用/02-历史草稿/07-杂项草稿与实验记录/00-最新版框架完整设计与实验方案.md`。
