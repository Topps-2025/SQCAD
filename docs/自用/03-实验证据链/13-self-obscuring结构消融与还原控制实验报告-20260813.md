...
> **Scope correction（2026-08-21）：** 本文的单世界 restore plateau、`O(1/(q rho))` 方向和旧 P4 matching 仅是机制诊断；不控制 `W-` false restore，也不构成当前安全上界。当前闭环以 Theorems 11–13 和 Certificate–Censoring Bridge B1–B4 为准。
...
| 参数 | 值 | 含义 |
|---|---|---|
| $\tau$ | 10 | keep 时暴露步的生命周期价值（K 世界 +10 / 步，A 世界 −10 / 步） |
| $p_{\mathrm{expose}}$ | 0.6 | keep 状态下的证据到达率 |
| $p_{\mathrm{evid\_archived}}$ | 0.6（W0/W1）／ 0.0（W2/W3） | archived 状态下的证据到达率——**审查开关** |
| $n_{\mathrm{early}}$ | 50 | 早期共因观测阶段（混杂先验，$\delta_{\mathrm{early}}=-0.5$，K/A 逐位共享） |
| $T$ | 2000 | 水平线 |
| $\theta_{\mathrm{keep}}$ / $\theta_{\mathrm{archive}}$ | +2.0 / −2.0 | 证据驱动的 keep / archive 阈值 |
| $\mathrm{prior\_sd}$ | 1.15 | 先验宽度（识别集加宽，12- Theorem 5） |
| $c_{\mathrm{probe}}$ / $c_{\mathrm{restore}}$ | 30 / 80 | 探测、恢复成本 |
| seeds | 12（21–32） | 每 cell 独立世界流 |

**精确斜率基准**：K 世界中被错误归档的记忆，从决策点起每个后续步以概率 $p=0.6$ 暴露并产生 $\tau=10$ 的错失价值。T1(a)（15- §2.2）预测精确线性斜率
...
|---|---|---|---|
| **W0**（query-local，不审查）K$\cdot$watchful_no_restore | **0.0440 ± 0.0098** | 14.7 | 证据自由流动 → watchful 修正（几乎零 regret） |
| W0 K$\cdot$association_commit | **5.8500**（精确） | 2000 | 不看证据的静态规则：即使证据流动也不修正 |
| W0 A$\cdot$association_commit | 0.0000 | 0 | 归档恰好正确 |
| **W1**（persistent，不审查）K$\cdot$watchful_no_restore | **0.0663 ± 0.0115** | 22.1 | 持久动作 + 未审查证据：watchful 仍修正 |
| **W2**（persistent，审查，无恢复）K$\cdot$watchful_no_restore | **5.8500**（精确） | 2000 | **自我证实**：审查使证据停止 → 永无修正机会 |
| W2 K$\cdot$association_commit | 5.8500（精确） | 2000 | 同上（两策略观测等价，见 §2） |
| **W3**（审查 + 恢复通道）K$\cdot$watchful_restore | **0.4250 ± 0.1441** | 141.7（付 80） | 恢复通道重开证据流 → regret 平台化 |
| W3 K$\cdot$watchful_no_restore | 5.8500（精确） | 2000 | 同一 watchful 策略、无通道 → 线性 |
| W2/W3 A 世界（全部） | 0.0000 | — | 归档正确侧：零 regret |

**读法**（对照 14- §7.1 的 W0–W3 语义）：

1. **审查是因果杠杆，策略不是**：同一 watchful 策略在 W0 修正（0.044）、在 W2 线性失败（5.85）。把环境从 query-local 换成 persistent 只引入少量额外噪声（W1 0.066），把证据流审查掉则定理从"修正"翻转为"$\Theta(T)$ regret"（T1(a) 的 W2 数值兑现）；
...
**6.1 UCB 共因早期流污染**（影响 W2$\cdot$candidate UCB）：UCB 把 $t<50$ 的共因观测史（K/A 逐位共享、均值 −0.5，无法区分生命周期符号）计入样本。$n=1$ 时 CI $\pm 1.96\sigma$，seed 31 的一次幸运抽样（$y>1.96$，$p \approx 0.007/\mathrm{seed}$；12 个 seed 恰好命中 1 个）在 $t=0$ 触发假翻转（corr=−50, slope=0）。修复：UCB 只消费 **post-decision 候选流**（早期流是观测等价构造的组成部分，见 §2）。修复后 W2 12/12 seed 精确 5.8500，W1 修正时间 1.3→1.2。

**6.2 uncertainty_triggered_restore 恢复环路**（影响 W3 A 世界）：无确认标志时，staleness 恢复（$\mathrm{bel} > -2.37$ 区域）与证据确认再归档（$\mathrm{bel} < -2.0$）的守卫互补，$\mathrm{bel} \in (-2.37, -2.0)$ 时两块在同一轮迭代内互触发——A 世界 restores=1745、成本 139593，状态轨迹却只有 3 次变化（同轮翻转被日志掩盖）。修复：恢复后再归档 ⇒ **沉默有信息**（harm_confirmed），不再触发恢复。修复后 A 世界恰一次恢复（成本 80，slope 0.0245），K 世界平台不变（0.2937/97.9）。
...
修正时间随 $q$ 单调递减（T1(b) 上界 $O(1/(q\rho))$ 的数值方向：15- §4 理论界 $\tau p/q = 600/120/30$，经验 corr 818/142/37，比值 1.1–1.4，常数因子内一致）；斜率随 $q$ 单调递减，平台由恢复成本主导。
...
1. T1：无恢复通道的 committing 策略在审查世界有精确线性 regret（数值 $5.8500 = \tau \cdot p \cdot (T-n_{\mathrm{early}})/T$ 逐位兑现）；恢复通道把 regret 界到 $O(1/q)$（与 $T$ 无关）；审查结构去掉后（W0）现象消失；
...
- 理论：`docs/自用/01-research-gap/研究逻辑与理论证明/15-self-obscuring形式定理与严格证明-20260813.md`（引理 1/2、定理 1/2 的严格证明、精确斜率 5.85 推导、O(1/q) 上界、sweep 预测对照）；
