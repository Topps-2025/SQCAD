...
2. **公开集复现完成，并定位到真实 bad case**：原 SQCAD 的主要短板是证据没有进入一次性暴露池，而不是资格层排序本身失效。最小修复 Guard-1 只将至多一个 BM25 候选加入当前读取池，不改变持久写入授权；LoCoMo token-F1 从 0.0339 提升到 0.0451，与 BM25（0.0454）持平，Guard-2/4 高于 BM25（0.0452/0.0469）。数值为 25- 修复后确定性结果。
...
| 原 SQCAD | 0.754 | 0.115 | 0.0339 | 持久存储低，但候选覆盖不足 |
| Guard-1 | 0.902 | 0.151 | 0.0451 | 推荐的最小 coverage 修复，F1 与 BM25 持平 |
| Guard-2 | 0.921 | 0.177 | 0.0452 | 更高覆盖，更多 probe |
| Guard-4 | 0.944 | 0.224 | 0.0469 | 覆盖更高，成本也更高 |
