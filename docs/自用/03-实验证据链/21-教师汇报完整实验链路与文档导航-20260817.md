...
| 4. 公开集 | 不泄漏金标时，候选覆盖、QA 与成本是否可用？ | Hit、recall、F1、token、probe、restore | [16-统一合同](16-公开数据集统一合同主表与显著性判定-20260814.md)、[19-AutoDL](19-AutoDL公开集复现与Candidate-Guard调整报告-20260816.md) | Guard-1 作为主配置；dense/RRF 待补 |
...
|---|---|---|---|
| 基线与公开集复现 | 已完成 | candidate guard 修复 coverage，Guard-1 是最小配置 | 不代表 SOTA；补 dense/RRF 或严格标注不可得 |
...
3. [19-AutoDL 公开集报告](19-AutoDL公开集复现与Candidate-Guard调整报告-20260816.md)：公开数据的复现、bad case 与 Guard-1。
