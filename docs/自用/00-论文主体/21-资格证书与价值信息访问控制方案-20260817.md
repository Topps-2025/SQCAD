...
```text
tau(i,s) = V_s^pi(keep_i) - V_s^pi(archive_i)
```
...
| `U < 0` | 授权 `archive` |
| `L <= 0 <= U` | 不提交持久动作；比较 `defer` 与 `probe` |
...
候选 probe 必须预注册预期的后验区间 `[L',U']` 和成本 `C_probe`：

```text
VOI = C_defer - (C_probe + R*(L', U'))
R*(L,U) = U(-L)/(U-L)    (only when L < 0 < U)
```

仅当 `VOI > 0` 时 probe；按 `VOI` 排序，检索分数仅用于同值打破。probe 产生的是一次性 exposure，除非更新后的资格证书授权，否则不能写 persistent store。
